...
function CuentasListPage() {
  ...
  const handleDelete = (id) => {
    if (window.confirm('¿Eliminar cuenta?')) {
      deleteCuenta(id).then(() => setCuentas(cuentas => cuentas.filter(c => c.id_cuenta !== id)));
    }
  };
...
function NuevaCuentaPage() {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    createCuenta(form)
      .then(() => navigate('/cuentas-bancarias'))
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="card p-4 shadow">
        ...
        <button className="btn btn-secondary mt-2" onClick={() => navigate('/cuentas-bancarias')}>Volver</button>
      </div>
    </div>
  );
}
...
function EditarCuentaPage() {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    updateCuenta(id, form)
      .then(() => navigate('/cuentas-bancarias'))
      .catch(() => alert('Error al actualizar cuenta'));
  };
